...
function action(input, output, filename) {
	...
	selectWindow(title1);
	run("8-bit");	
	run("Merge Channels...", "c1=[C1-"+toString(title3)+"] c2=[C2-"+toString(title3)+"] c3=[C3-"+toString(title3)+"] c4=["+toString(title1)+"]");
	save(output + "Kmeans_" +title1); 
	close("*");
	*/
	//title1=getTitle();
//title2="BackGD_Kmeans"+substring(title1,4)+"-KmeansCluster.tif";
